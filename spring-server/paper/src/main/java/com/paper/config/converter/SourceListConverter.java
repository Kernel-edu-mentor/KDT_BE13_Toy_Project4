...
@Slf4j
@Converter
public class SourceListConverter implements AttributeConverter<List<QAResponse.Source>, String> {

    private final ObjectMapper objectMapper = new ObjectMapper();
    // Jackson이 List<Source> 타입을 인식하도록 TypeReference 사용
    private final com.fasterxml.jackson.core.type.TypeReference<List<QAResponse.Source>> typeRef =
            new com.fasterxml.jackson.core.type.TypeReference<List<QAResponse.Source>>() {};

    /**
     * 엔티티 필드 (List<Source>) -> DB 컬럼 (String/JSONB)으로 변환
     */
    @Override
    public String convertToDatabaseColumn(List<QAResponse.Source> attribute) {
        if (attribute == null) {
            return null;
        }
        try {
     ...
     */
    @Override
    public List<QAResponse.Source> convertToEntityAttribute(String dbData) {
